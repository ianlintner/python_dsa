...
def fix_testcase_in_file(file_path):
    ...
    content = re.sub(r'input_args=\(([^)]+),\s*\)', fix_tuple_pattern, content, flags=re.MULTILINE | re.DOTALL)
    ...
    content = re.sub(r'(TestCase\([^)]+\)),\s*([^,\n]+),?\s*([^,\n]*)\)', fix_positional_after_keyword, content, flags=re.MULTILINE | re.DOTALL)
