...
def demo():
    ...
    s = "abcabcbb"
    print(f"Longest substring without repeating in '{s}': {length_of_longest_substring_without_repeating(s)}")
    s2 = "ADOBECODEBANC"
    t2 = "ABC"
    ...
    flips = 2
    print(f"Longest ones with at most {flips} zero flips in {bin_arr}: {longest_ones_with_k_zero_flips(bin_arr, flips)}")
    print()
    
    nonneg = [1,2,1,0,1,1,0]
    at_most = 4
    print(f"Max length subarray with sum <= {at_most} in {nonneg}: {subarray_max_sum_at_most_k(nonneg, at_most)}")
    print()
    ...
    print("  - For sums with negative numbers, sliding window breaks; use prefix sums and other techniques.")
