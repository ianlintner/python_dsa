...
class SegmentTree:
    ...
    def _range_sum(self, idx: int, l: int, r: int, ql: int, qr: int) -> int:
        ...
        right_len = r - mid
        self._push(idx, left_len, right_len)
        return self._range_sum(idx << 1, l, mid, ql, qr) + self._range_sum(idx << 1 | 1, mid + 1, r, ql, qr)
...
class RangeMinSegmentTree:
    ...
    def range_min(self, ql: int, qr: int) -> int:
        ...
        while ql <= qr:
            if ql & 1:
                res = min(res, self.tree[ql]); ql += 1
            if not (qr & 1):
                res = min(res, self.tree[qr]); qr -= 1
            ql >>= 1; qr >>= 1
        return res
...
def demo():
    ...
    print("  - For range assign + range sum, maintain both 'assign' and 'add' lazy tags with priority")
    print("  - Iterative segment trees are compact and fast, but recursion is easier to reason about")
