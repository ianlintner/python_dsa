...
def dijkstra_with_path(graph: WeightedGraph, source: Vertex, target: Optional[Vertex] = None) -> Tuple[Dict[Vertex, Weight], Dict[Vertex, Optional[Vertex]]]:
    """
...
def reconstruct_path(parents: Dict[Vertex, Optional[Vertex]], source: Vertex, target: Vertex) -> Optional[List[Vertex]]:
    """
...
def dijkstra_single_target(graph: WeightedGraph, source: Vertex, target: Vertex) -> Tuple[Weight, Optional[List[Vertex]]]:
    """
...
def dijkstra_k_shortest_paths(graph: WeightedGraph, source: Vertex, target: Vertex, k: int) -> List[Tuple[Weight, List[Vertex]]]:
    """
    ...
    def find_paths_dfs(current: Vertex, target: Vertex, path: List[Vertex], 
                      cost: Weight, visited: Set[Vertex], all_paths: List[Tuple[Weight, List[Vertex]]]):
        if len(all_paths) >= k:
            return
...
def find_shortest_path_with_constraints(graph: WeightedGraph, source: Vertex, target: Vertex, 
                                      max_stops: int) -> Tuple[Weight, Optional[List[Vertex]]]:
    """
