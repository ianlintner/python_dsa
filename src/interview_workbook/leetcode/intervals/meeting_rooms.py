...
def create_demo_output() -> str:
    ...
    examples = [
        ...
        {"input": [[7, 10], [2, 4]], "description": "Non-overlapping meetings - can attend all"},
        {"input": [], "description": "No meetings - can attend all"},
        {"input": [[1, 5]], "description": "Single meeting - can attend"},
        {"input": [[1, 4], [4, 5]], "description": "Adjacent meetings (touching) - can attend"},
        {"input": [[1, 5], [2, 3], [4, 6]], "description": "Multiple overlaps - cannot attend"},
    ]
        ...
        if original_intervals:
            ...
            for j in range(1, len(sorted_intervals)):
                if sorted_intervals[j - 1][1] > sorted_intervals[j][0]:
                    overlaps.append(
                        f"{sorted_intervals[j - 1]} overlaps with {sorted_intervals[j]}"
                    )
