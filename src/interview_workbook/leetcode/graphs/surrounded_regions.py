...
class Solution:
    ...
    def solve_bfs_border_escape(self, board: list[list[str]]) -> None:
        ...
        for row in range(m):
            for col in range(n):
                if (row == 0 or row == m - 1 or col == 0 or col == n - 1) and board[row][
                    col
                ] == "O":
...
def create_demo_output() -> str:
    ...
    examples = [
        ...
        {"input": [["X"]], "description": "Single cell edge case"},
        {
    ...
    for i, example in enumerate(examples, 1):
        ...
        output.append(
            f"BFS result (should match): {board_to_string(board_bfs) == board_to_string(board_dfs)}"
        )
        ...
        output.append(
            f"Union-Find result (should match): {board_to_string(board_uf) == board_to_string(board_dfs)}"
        )
        output.append("")
