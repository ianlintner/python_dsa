...
class Solution:
    ...
    def pacificAtlanticBruteForce(self, heights: list[list[int]]) -> list[list[int]]:
        ...
        for i in range(m):
            for j in range(n):
                if (can_reach_ocean(i, j, "pacific", set()) and 
                    can_reach_ocean(i, j, "atlantic", set())):
                    result.append([i, j])
...
def create_demo_output() -> str:
    ...
    demo_cases = [
        ...
        {
            "name": "Example 2: Uniform Heights", 
            "heights": [[1,1],[1,1]]
        },
        {
            "name": "Example 3: Single Cell",
            "heights": [[1]]
        },
        {
            "name": "Example 4: Diagonal Gradient",
            "heights": [
                [1,2,3],
                [2,3,4], 
                [3,4,5]
