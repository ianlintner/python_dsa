...
class Solution:
    ...
    def findOrderLexicographical(self, numCourses: int, prerequisites: List[List[int]]) -> List[int]:
        """
...
def test_solution():
    ...
    def is_valid_topological_order(order: List[int], num_courses: int, prerequisites: List[List[int]]) -> bool:
    ...
    def test_function(numCourses, prerequisites):
        ...
        return result if is_valid_topological_order(result, numCourses, prerequisites) else "INVALID"
    ...
    for case in modified_cases:
        ...
        expected_valid = len(case.expected_output) > 0 or (len(case.expected_output) == 0 and "impossible" in case.description.lower())
