...
def create_demo_output() -> str:
    ...
    output.append("2. **Greedy Strategy**: Local optimal choice (heaviest stones) leads to global optimum")
...
def simulate_stone_smashing(stones: List[int]) -> List[str]:
    ...
    while len(stones_copy) > 1:
        ...
        if first == second:
            ...
            stones_copy.sort(reverse=True)
            steps.append(f"Smash {first} and {second} → new stone {new_stone}, remaining: {stones_copy}")
...
def test_solution():
    ...
    run_test_cases(TEST_CASES, lambda tc: solution.lastStoneWeight(
        tc.input_data["stones"]
    ))
