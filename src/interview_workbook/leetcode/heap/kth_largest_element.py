...
def create_demo_output() -> str:
    ...
    output.append("1. **Heap Selection**: Use min-heap when k is small, max-heap for multiple queries")
...
def test_solution():
    ...
    run_test_cases(TEST_CASES, lambda tc: solution.findKthLargest(
        tc.input_data["nums"].copy(), tc.input_data["k"]
    ))
