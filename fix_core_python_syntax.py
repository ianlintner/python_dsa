...
def fix_basic_python_syntax(content: str) -> str:
    ...
    content = re.sub(r'^(\s*)def\s+([A-Za-z_][A-Za-z0-9_]*)\s*\([^)]*\)\s*=\s*',
                     lambda m: m.group(0).replace('=', ':'), content, flags=re.MULTILINE)

    # Fix method definitions with return types: "def method(self, args) -> ReturnType="
    content = re.sub(r'^(\s*)def\s+([A-Za-z_][A-Za-z0-9_]*)\s*\([^)]*\)\s*->\s*[^=]+\s*=\s*$',
    ...
    content = re.sub(r'^(\s*)for\s+([^=]+)\s+in\s+([^=]+)\s*=\s*$', r'\1for \2 in \3:', content, flags=re.MULTILINE)
